...
void updateChildData(objects) {
	...
	traceEvent(settings.logFilter,"updateChildData>data.sensors=${data?.sensors}",settings.trace,get_LOG_TRACE())        
}
...
private def refresh_sensor(sensorId="") {
	...
	state?.scale= scale
	def tstat_scale = data?.sensors[0]?.temperature_scale
	if (tstat_scale && tstat_scale != scale) {
    
		traceEvent(settings.logFilter,"refresh_sensor>your SmartThings scale ($scale) is different from your Nest Tstat's scale ($tstat_scale), please correct it",settings.trace, get_LOG_WARN())
...
//		humidity:data?.sensors[0]?.humidity,
		...
		"tstat_scale": scale,
		"has_fan":  data?.sensors[0]?.has_fan.toString(),
	...
	api('setSensorSettings', sensorId, bodyReq) {resp ->
		...
		if (statusCode== REDIRECT_ERROR) {
			if (!process_redirectURL( resp?.headers.Location)) {
				traceEvent(settings.logFilter,"setSensorSettings>Nest redirect: too many redirects, count =${state?.redirectURLcount}", true, get_LOG_ERROR())
				return                
			}
		...
		if (statusCode==BLOCKED) {
			traceEvent(settings.logFilter,"setSensorSettings>sensorId=${sensorId},Nest throttling in progress, error $statusCode, retries={state?.retriesSettingsCounter}", settings.trace, get_LOG_ERROR())
			interval=1*60 			   // set a minimum of 5min. interval to avoid unecessary load on Nest servers
		}			
		if (statusCode==TOKEN_EXPIRED) {
			traceEvent(settings.logFilter,"setSensorSettings>error $statusCode, need to re-login at Nest",true, get_LOG_WARN())
			return            
		}
		...
		if (statusCode == NEST_SUCCESS) {
			...
			traceEvent(settings.logFilter,"setSensorSettings> error=${statusCode.toString()} for ${sensorId}", true, get_LOG_ERROR())
		} /* end if statusCode */
	} /* end api call */                
	if (exceptionCheck?.contains("exception")) {
		sendEvent(name: "verboseTrace", value: "", displayed:(settings.trace?:false)) // reset verboseTrace            
		traceEvent(settings.logFilter,"setSensorSettings>exception=${exceptionCheck}", true, get_LOG_ERROR())
	}                
	if ((statusCode == BLOCKED) ||
		...
		if (!(interval= get_exception_interval_delay(state?.retriesSettingsCounter))) {   
			traceEvent(settings.logFilter,"SetsensorSettings>too many retries", true, get_LOG_ERROR())
			state?.retriesSettingsCounter=0 
			reset_replay_data('Settings')                
			return        
		}        
		...
		traceEvent(settings.logFilter,"setSensorSettings>about to call setSensorSettingsReplay,retries counter=${state?.retriesSettingsCounter}", true, get_LOG_INFO())
...
void setSensorSettingsReplay() {
	...
	for (int i=1; (i<= get_MAX_SETTER_RETRIES()); i++) {
		...
		traceEvent(settings.logFilter,"setSensorSettingsReplay>about to recall setSensorSettings for sensorId=$sensorId,retries counter=$i" +
			",sensorSettings=$sensorSettings", true, get_LOG_INFO())
...
void getsensorInfoReplay() {
	...
	def poll_interval=1 
	state?.lastPollTimestamp= (now() - (poll_interval * 60 * 1000)) // reset the lastPollTimeStamp to pass through
	traceEvent(settings.logFilter,"getsensorInfoReplay>about to call getsensorInfo for ${id}",settings.trace, get_LOG_INFO())
	getsensorInfo(id) 
}    
