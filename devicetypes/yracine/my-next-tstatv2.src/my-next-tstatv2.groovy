...
metadata {
	...
	definition(name: "My Next TstatV2", author: "Yves Racine", ocfDeviceType:"oid.d.thermostat" ,namespace: "yracine") {
		capability "Sensor"
		capability "Actuator"
...
//		command "getThermostatList"
		...
		command "setThermostatSettings" // signature setThermostatSettings(thermostatId,String thermostatSettings), thermostatSettings must be a valid json map
...
//		new attributes in V2
		...
		attribute "away_temperature_high", "string"
		attribute "away_temperature_low", "string"
		...
		attribute "emer_heat_source", "string"
		attribute "emer_heat_delivery", "string"
		attribute "heat_pump_aux_threshold", "string"
		...
		attribute "heat_pump_comp_threshold", "string"
		...
		attribute "upper_safety_temp", "string"
		attribute "lower_safety_temp_enabled", "string"
		attribute "lower_safety_temp", "string"
...
private def fanActive() {
	long timeout=device.currentValue("fan_timer_timeout")?.toLong()
	traceEvent(settings.logFilter, "fanActive>timeout=$timeout", settings.trace)
	def result=(timeout !=0) ? true :false 
	traceEvent(settings.logFilter, "fanActive>timeout=$timeout, result=$result", settings.trace)
	return result
}
...
void setHeatingSetpoint(temp) {	
	...
	if ((scale!='C') && (coolSP.toDouble()  < heatingSetpointRangeLow.toDouble())) {
...
void setCoolingSetpoint(temp) {
	...
	if ((scale!='C') && (heatSP.toDouble()  < coolingSetpointRangeLow.toDouble())) {
...
private def refresh_thermostat(thermostatId="") {
	...
	if (tstat_scale != scale) {
		...
		traceEvent(settings.logFilter,"refresh_thermostat>your SmartThings scale ($scale) is different from your Nest Tstat's scale ($tstat_scale), please correct it",settings.trace, get_LOG_WARN())
		scale=tstat_scale        
	...
	if (data?.thermostats[0]?.temperature_lock) { 
		...
		heatingSetpointRangeLow= (scale=='C')? (data?.thermostats[0]?.temperature_lock_low_temp):(cToF_Rounded(data?.thermostats[0]?.data?.thermostats[0]?.temperature_lock_low_temp))
		...
		coolingSetpointRangeLow= (scale=='C')? (data?.thermostats[0]?.temperature_lock_low_temp):(cToF_Rounded(data?.thermostats[0]?.data?.thermostats[0]?.temperature_lock_low_temp))
		heatingSetpoint=(scale=='C')? (data?.thermostats[0]?.temperature_lock_low_temp):(cToF(data?.thermostats[0]?.data?.thermostats[0]?.temperature_lock_low_temp))
...
//		"previous_hvac_mode": data?.thermostats[0]?.previous_hvac_mode,
...
//		"last_connection": (data?.thermostats[0]?.last_connection)? formatDateInLocalTime(data?.thermostats[0]?.last_connection)?.substring(0,16):"",
...
//		"hvac_state": data?.thermostats[0]?.hvac_state,
		...
		"away_temperature_high_c": (data?.thermostats[0]?.away_temperature_enabled) ? data?.thermostats[0]?.away_temperature_high : 0,
		"away_temperature_high_f": (data?.thermostats[0]?.away_temperature_enabled) ? cToF_Rounded(data?.thermostats[0]?.away_temperature_high) : 0, 
 		"away_temperature_low_c": (data?.thermostats[0]?.away_temperature_enabled) ? data?.thermostats[0]?.away_temperature_low : 0, 
		"away_temperature_low_f": (data?.thermostats[0]?.away_temperature_enabled) ? cToF_Rounded(data?.thermostats[0]?.away_temperature_low) : 0, 
		...
		"hvac_heater_state": data?.thermostats[0]?.hvac_heater_state,
 		"auto_away": data?.thermostats[0]?.auto_away,
...
//		"backplate_temperature": data?.thermostats[0]?.backplate_temperature,
		...
		"lower_safety_temp": (data?.thermostats[0]?.lower_safety_temp_enabled) ? data?.thermostats[0]?.lower_safety_temp :10,
		"upper_safety_temp_enabled": data?.thermostats[0]?.upper_safety_temp_enabled,
		"upper_safety_temp": (data?.thermostats[0]?.upper_safety_temp_enabled) ? data?.thermostats[0]?.upper_safety_temp :30,
...
void setThermostatSettings(thermostatId,String thermostatSettings) {
	...
	try {
		...
		traceEvent(settings.logFilter,"setThermostatSettings>was able to convert json settings  to a map",settings.trace, get_LOG_INFO())
	} catch (e) {
		traceEvent(settings.logFilter,"setThermostatSettings>was not able to convert settings {$thermostatSettings) to a map, not a valid json map",settings.trace, get_LOG_ERROR())
		return
	}
...
void setThermostatSharedSettings(thermostatId,tstatSettings = []) {
	...
	api('setThermostatSettings', thermostatId, bodyReq) {resp ->
		...
		if (statusCode==BLOCKED) {
			traceEvent(settings.logFilter,"setThermostatSharedSettings>thermostatId=${thermostatId},Nest throttling in progress, error $statusCode, retries={state?.retriesSharedSettingsCounter}", settings.trace, get_LOG_ERROR())
			interval=1*60 			   // set a minimum of 5min. interval to avoid unecessary load on Nest servers
		}			
		if (statusCode==TOKEN_EXPIRED) {
			traceEvent(settings.logFilter,"setThermostatSharedSettings>error $statusCode, need to re-login at Nest",true, get_LOG_WARN())
			return            
		}
		exceptionCheck=device.currentValue("verboseTrace")
		if (statusCode == NEST_SUCCESS) {
			...
			traceEvent(settings.logFilter,"setThermostatSharedSettings> error=${statusCode.toString()} for ${thermostatId}", true, get_LOG_ERROR())
		} /* end if statusCode */
	} /* end api call */                
	if (exceptionCheck?.contains("exception")) {
		sendEvent(name: "verboseTrace", value: "", displayed:(settings.trace?:false)) // reset verboseTrace            
		traceEvent(settings.logFilter,"setThermostatSharedSettings>exception=${exceptionCheck}", true, get_LOG_ERROR())
	}                
	if ((statusCode == BLOCKED) ||
		(exceptionCheck?.contains("Nest response error")) || 
		(exceptionCheck?.contains("ConnectTimeoutException"))) {
		...
		if (!(interval= get_exception_interval_delay(state?.retriesSharedSettingsCounter))) {   
			traceEvent(settings.logFilter,"setThermostatSharedSettings>too many retries", true, get_LOG_ERROR())
			state?.retriesSharedSettingsCounter=0 
			reset_replay_data('SharedSettings')                
			return        
		}        
		...
		traceEvent(settings.logFilter,"setThermostatSharedSettings>about to call setThermostatSharedSettingsReplay,retries counter=${state?.retriesSharedSettingsCounter}", true, get_LOG_INFO())
...
//	traceEvent(settings.logFilter,"sethold>targetCoolTemp=$targetCoolTemp & targetHeatTemp=targetHeatTemp", settings.trace)
	...
	if (scale == 'C') {
		if (currentMode == 'auto' && canHeat && canCool) {
			...
			} else if (targetCoolTemp.toDouble() != current_temp_high) {
...
void updateChildData(objects) {
	...
	traceEvent(settings.logFilter,"updateChildData>data?.thermostats=${data?.thermostats}",settings.trace,get_LOG_TRACE())        
}
