...
//	app.updateSetting(settingKey,5)  
	...
	if (!atomicState.accessToken) {
		...
		if ((!atomicState?.nest_user_id) && (!atomicState?.google_issue_token_url)) {
			traceEvent(settings.logFilter,"authPage>error: the smartapps's nest_user_id or google_issue_token_url appSettings variables are not populated, please read the README at the github", true, get_LOG_ERROR(), true)
...
def structureList() {
	...
	traceEvent(settings.logFilter,"structureList>about to call, type=[where]", detailedNotif,get_LOG_TRACE())
	structures=getStructures(false, settings.cache_timeout,'["where"]')
	def known_buckets= get_KNOWN_BUCKET_TYPES()   
	traceEvent(settings.logFilter,"structureList>about to call, type=$known_buckets", detailedNotif,get_LOG_TRACE())
	structures=getStructures(false, settings.cache_timeout,"${known_buckets}")
/*    
	if (atomicState?.access_token) {
    
		traceEvent(settings.logFilter,"otherSettings>about to call login_dropcam with ${atomicState?.access_token}", detailedNotif,get_LOG_TRACE())
    	if (login_dropcam(atomicState?.access_token, atomicState?.cookie)) {
				getAllCameras(false)
		} else {
			traceEvent(settings.logFilter,"otherSettings>login_dropcam failed,atomicState?.lastHttpStatus=${atomicState?.lastHttpStatus}", detailedNotif,get_LOG_TRACE(), true)
...
def NextTstatList() {
	...
	if (tstatObjects) {     
		tstatObjects.each {    
			traceEvent(settings.logFilter,"NextTstatList>> about to loop returned thermostats from getObject(): $tstatObjects", detailedNotif,get_LOG_TRACE())    
		...
		traceEvent(settings.logFilter,"NextTstatList>> about to get protect list from previously stored thermostats", detailedNotif,get_LOG_TRACE())    
		thermostats.each {
...
def NextSensorList() {
	...
	traceEvent(settings.logFilter,"NextSensorList> about to call getObject()", detailedNotif, get_LOG_TRACE())
	...
	if (sensorObjects) {     
		sensorObjects.each {    
			traceEvent(settings.logFilter,"NextSensorList>> about to loop returned thermostats from getObject(): $tstatObjects", detailedNotif,get_LOG_TRACE())    
			def name = (it?.name) ?: it?.long_name                    
			def dni = [ app.id, name, it?.id].join('.')
			sensorDNIs[dni] = name
		}  
		...
		traceEvent(settings.logFilter,"NextSensorList> about to get sensor list from previously stored sensors", detailedNotif,get_LOG_TRACE())    
		sensors.each {
...
def NextProtectList() {
	...
	def object =[:]   
	if (protectObjects) {    
		traceEvent(settings.logFilter,"NextProtectList>> about to loop returned protects from getObject(): $protectObjects", detailedNotif,get_LOG_TRACE())    
		protectObjects.each {    
			def name = (it?.name) ?: it?.long_name                    
			def dni = [ app.id, name, it?.id].join('.')
		...
		traceEvent(settings.logFilter,"NextProtectList>> about to get protect list from previously stored protects", detailedNotif,get_LOG_TRACE())    
		protectUnits.each {
...
def NextCamList() {
	...
	def object =[:]   
	if (cameraObjects) {    
		traceEvent(settings.logFilter,"NextCamList>> about to loop returned cams from getObject(): $cameraObjects", detailedNotif,get_LOG_TRACE())    
		cameraObjects.each {    
			def name = (it?.name) ?: it?.long_name                    
			def dni = [ app.id, name, it?.id].join('.')
			cameraDNIs[dni] = name
		...
		traceEvent(settings.logFilter,"NextCamList>> about to get camera list from previously stored cameras", detailedNotif,get_LOG_TRACE())    
		cameras.each {
...
//	def args_encoded = java.net.URLEncoder.encode(args.toString(), "UTF-8")
	...
	traceEvent(settings.logFilter,"login_dropcam>data=$args", detailedNotif,get_LOG_TRACE(), send_msg)
	def loginParams = [
	...
	traceEvent(settings.logFilter,"login_dropcam>About to post $loginParams", detailedNotif,get_LOG_TRACE(), send_msg)
	try {
		httpPost(loginParams) { resp ->
			atomicState?.lastHttpStatus=resp?.status
			traceEvent(settings.logFilter,"login_dropcam>resp.status=${resp?.status}", detailedNotif, get_LOG_TRACE(), send_msg)
			traceEvent(settings.logFilter,"login_dropcam>resp.status=${resp?.status}", detailedNotif, get_LOG_TRACE(), send_msg)
			if (resp?.status == NEST_SUCCESS) {
				traceEvent(settings.logFilter,"login_dropcam>success, resp.status=${resp?.status}", detailedNotif)
				traceEvent(settings.logFilter, "login_dropcam>success, resp.data=${resp?.data}",detailedNotif,get_LOG_TRACE(), send_msg)
			} else {
		...
		traceEvent(settings.logFilter,msg, true, get_LOG_ERROR(), send_msg)
		return false        
		...
		traceEvent(settings.logFilter,"login_dropcam>exception $e", detailedNotif, get_LOG_TRACE(), send_msg)
		traceEvent(settings.logFilter,"login_dropcam>exception, resp.status=${e?.response?.status}", detailedNotif, get_LOG_TRACE(), send_msg)
		return false        
	}        
	return true    
...
private def getAllCameras(useCache=true, cache_timeout=2) {
	...
	cameras.each {
		...
		traceEvent(settings.logFilter,"getAllCameras>request_url=${request_url}", detailedNotif,get_LOG_TRACE())
		...
		try {
			...
			if (atomicState?.access_token) {  // if authenticated, then there is a problem.           
				atomicState?.lastHttpStatus=e?.response?.status
				traceEvent(settings.logFilter,"MyNextManager>error (${e}) while fetching cameras from Nest,trying to re-login", detailedNotif,get_LOG_ERROR())     
			}  
...
def getStructures(useCache=true, cache_timeout=2, type='["buckets"]') {
//	settings.detailedNotif=true // set to true initially
//	settings.logFilter=3   
	boolean found_in_cache=false
	def REFERER= get_REFERER()
	def USER_AGENT=get_USER_AGENT()    
    
	traceEvent(settings.logFilter,"getStructures>begin fetching ${type}...", detailedNotif,get_LOG_TRACE())
	def msg
 	def TOKEN_EXPIRED=401
	def NEST_SUCCESS=200    
	...
	if (useCache) {    
		...
		if ((atomicState?."${type}") && ((cache_timestamp) && (cache_timestamp > time_check_for_cache))) {  //cache still valid
			...
			if ((type == get_KNOWN_BUCKET_TYPES()) || (type == '["buckets"]')) {
				def cacheDeviceData=[:]
	...
	if (!responseValues) {      
		...
		traceEvent(settings.logFilter,"getStructures>device list params: $deviceListParams",detailedNotif,get_LOG_TRACE())

		if ((atomicState?.google_issue_token_url) && (isTokenExpired())) {
			traceEvent(settings.logFilter,"getStructures>token expired, calling login()",detailedNotif,get_LOG_TRACE())
			login()			        
		}        
		try {
			httpPost(deviceListParams) { resp ->
				...
				traceEvent(settings.logFilter,"getStructures>resp.data=${resp?.data}", detailedNotif, get_LOG_TRACE())
//				traceEvent(settings.logFilter, "getStructures>resp.data.buckets=${resp?.data?.updated_buckets[0]}",detailedNotif, get_LOG_TRACE())
				if (resp?.status == NEST_SUCCESS) {
					...
					traceEvent(settings.logFilter, "getStructures>nest_czfe_url=${atomicState?.nest_czfe_url}",detailedNotif, get_LOG_TRACE())
				} else {
					traceEvent(settings.logFilter,"getStructures>http status: ${resp.status}",detailedNotif, get_LOG_TRACE())
					...
					//refresh the auth token
						traceEvent(settings.logFilter,"getStructures>http status=${resp?.status}: need to re-authorize at Nest, trying to call login()", 
								detailedNotif, get_LOG_WARN())      
						if (login()) { 
							refreshAllChildAuthTokens()
						}               
                    
					} else {
						traceEvent(settings.logFilter,"MyNextManager>error (${resp?.status}) while fetching structures from Nest", detailedNotif,get_LOG_WARN())     
					}
				} 
			}                
		} catch (java.net.UnknownHostException e) {
			...
			if ((e?.response?.status == TOKEN_EXPIRED) && (atomicState?.google_issue_token_url)) { // if authenticated, then there is a problem. 
				...
				traceEvent(settings.logFilter,msg, true,get_LOG_INFO()) 
				if (login()) { 
					refreshAllChildAuthTokens()
				}               
			}        
	...
	def deviceData=[:]
	def wheresData=[]
	def cameraList=[],tstatList=[],protectList=[],sensorList=[]
	...
	if (atomicState?.cameraList && type != '["buckets"]') {
		cameraList =  atomicState?.cameraList   
	}    
	if (atomicState?.thermostatList && type != '["buckets"]') {
		tstatList =  atomicState?.thermostatList   
	}    
	if (atomicState?.sensorList && type != '["buckets"]') {
		sensorList =  atomicState?.sensorList   
	}    
	if (atomicState?.protectList && type != '["buckets"]') {
...
//			traceEvent(settings.logFilter, "getStructures>object.inspect():${object?.value?.inspect()}", detailedNotif)
			...
			traceEvent(settings.logFilter, "getStructures>object=${object}", detailedNotif, get_LOG_TRACE())
		}        
		if (type=='["buckets"]') {    
			if (object.toString().startsWith('topaz.')) {
				...
				protectList << sn               
				traceEvent(settings.logFilter, "getStructures>id = $sn, objectType=Protect", detailedNotif, get_LOG_TRACE())
				...
				deviceData[sn] = (old_data)? (old_data + data): data                
				sensorList << sn                
				traceEvent(settings.logFilter, "getStructures>id = $sn, objectType=sensor", detailedNotif, get_LOG_TRACE())
				...
				deviceData[sn] = (old_data)? (old_data + data): data                
				tstatList << sn                
				traceEvent(settings.logFilter, "getStructures>id = $sn, objectType=thermostat", detailedNotif, get_LOG_TRACE())
                
			} else if (object.toString().startsWith('quartz.')) {
				sn = object.toString().tokenize('.').last()
				traceEvent(settings.logFilter, "getStructures>id = $sn, objectType=camera", detailedNotif, get_LOG_TRACE())
			...
			if (foundObject) {
				traceEvent(settings.logFilter, "getStructures>about to process objectId = $sn,  object_Key=$object_key, objectType=$deviceType", detailedNotif)
				if (object?.value instanceof Map) {            
					Map m = (Map) object_data
					m.each {k,v -> 
        		        if (detailedNotif) {
							traceEvent(settings.logFilter, "getStructures>${deviceType} ${sn}->${k}: ${v}\n", detailedNotif, get_LOG_TRACE())
						}
/*                        
						if (v!= null) {                     
							data << ["${k}": v]
...
*/                        
				...
				traceEvent(settings.logFilter, "getStructures>processing objectId = $sn,  object_Key=$object_key, objectType=$deviceType, data=${object_data}", detailedNotif,get_LOG_TRACE())
				if (object_key.toString().startsWith("shared.${sn}")) {
					...
					traceEvent(settings.logFilter, "getStructures>id = $sn, objectType=shared,  data=${deviceData[sn]}", detailedNotif,get_LOG_TRACE())
					...
					deviceData[sn] = (old_data)? (old_data + new_data): new_data                
					traceEvent(settings.logFilter, "getStructures>id = $sn, objectType=Protect,  data=${deviceData[sn]})", detailedNotif,get_LOG_TRACE())
					...
					deviceData[sn] = (old_data)? (old_data + new_data): new_data                
					traceEvent(settings.logFilter, "getStructures>id = $sn, objectType=tstat, data=${deviceData[sn]}", detailedNotif,get_LOG_TRACE())
	...
	if ((responseValues) && (!found_in_cache)  && (atomicState?.lastHttpStatus == NEST_SUCCESS)) {    
		...
		atomicState?.lastExecutionTimestamp = now() 
		if (type == '["buckets"]') {
			traceEvent(settings.logFilter, "getStructures>saving lists & deviceData", detailedNotif,get_LOG_TRACE())
			...
			traceEvent(settings.logFilter, "getStructures>saving wheresData", detailedNotif,get_LOG_TRACE())
			atomicState?.wheresData=wheresData    
		} else {           
			traceEvent(settings.logFilter, "getStructures>saving deviceData", detailedNotif,get_LOG_TRACE())
			atomicState?.deviceData=deviceData			        
		}
	}        
 
	traceEvent(settings.logFilter, "getStructures>end...objectType=Protects, List=${atomicState?.protectList}", detailedNotif,get_LOG_TRACE())
	traceEvent(settings.logFilter, "getStructures>end.. objectType=Thermostats, List=${atomicState?.thermostatList}", detailedNotif,get_LOG_TRACE())
	traceEvent(settings.logFilter, "getStructures>end.. objectType=Sensors, List=${atomicState?.sensorList}", detailedNotif,get_LOG_TRACE())
	traceEvent(settings.logFilter, "getStructures>end...objectType=Cameras, List=${atomicState?.cameraList}\n\n", detailedNotif,get_LOG_TRACE())
	traceEvent(settings.logFilter, "getStructures>end...deviceData=${atomicState?.deviceData}", detailedNotif,get_LOG_TRACE())
	return deviceData
}

private def save_shared_data(sn,object_data) {
	traceEvent(settings.logFilter, "save_shared_data>begin...about to save shared data...", detailedNotif,get_LOG_TRACE())
	def wheresData=[]
	if (atomicState?.wheresData) {
		wheresData=atomicState?.wheresData    
	}    
	...
	traceEvent(settings.logFilter, "save_shared_data>end...saved shared data...", detailedNotif,get_LOG_TRACE())
	return data
}

private def save_protect_data(sn, object_data) {
	traceEvent(settings.logFilter, "save_protect_data>begin...about to save protect data...", detailedNotif,get_LOG_TRACE())
	...
	traceEvent(settings.logFilter, "save_protect_data>end...saved protect data...", detailedNotif,get_LOG_TRACE())
	return data    
}

private def save_sensor_data(sn, object_data) {
	traceEvent(settings.logFilter, "save_sensor_data>begin...about to save sensor data...", detailedNotif,get_LOG_TRACE())
	...
	traceEvent(settings.logFilter, "save_sensor_data>end...saved sensor data...", detailedNotif,get_LOG_TRACE())
	return data    
}    

private def save_tstat_data(sn,object_data) {
	traceEvent(settings.logFilter, "save_tstat_data>begin...about to save tstat data...", detailedNotif,get_LOG_TRACE())
...
*/    
	...
	traceEvent(settings.logFilter, "save_tstat_data>begin...saved tstat data...", detailedNotif,get_LOG_TRACE())
	return data
}    

private def save_camera_data(sn, object_data) {
	traceEvent(settings.logFilter, "save_camera_data>begin...about to save camera data...", detailedNotif,get_LOG_TRACE())
	...
	traceEvent(settings.logFilter, "save_camera_data>end...saved camera data...", detailedNotif,get_LOG_TRACE())
	return data
}    
...
//	settings.detailedNotif=true // set to true initially
	...
	traceEvent(settings.logFilter,"getObject>begin fetching $relatedType", detailedNotif,get_LOG_TRACE())
	def msg
	def objects = []
	def type=get_KNOWN_BUCKET_TYPES()   

	def foundObject=null    
	if (useCache) {
    
		def known_buckets= get_KNOWN_BUCKET_TYPES()   
		traceEvent(settings.logFilter,"getObject>about to call getStructures(), type=known_buckets", detailedNotif,get_LOG_TRACE())
		deviceData=getStructures(true, cache_timeout,"${known_buckets}")
	} else {	
		traceEvent(settings.logFilter,"getObject>calling getStructures() to get all structures, not from cache", detailedNotif,get_LOG_TRACE())
		def structures
		atomicState?.thermostatList=[]        
		atomicState?.protectList=[]        
		atomicState?.sensorList=[]        
		atomicState?.cameraList=[]        
		traceEvent(settings.logFilter,"getObject>about to call getStructures(), type=buckets", detailedNotif,get_LOG_TRACE())
		structures=getStructures(false, cache_timeout,'["buckets"]')
		traceEvent(settings.logFilter,"getObject>about to call getStructures(), type=known_buckets", detailedNotif,get_LOG_TRACE())
		def known_buckets=get_KNOWN_BUCKET_TYPES()        
		deviceData=getStructures(false, settings.cache_timeout,"${known_buckets}")
	}        
   
	traceEvent(settings.logFilter,"getObject>id=${objectId}, relatedType=$relatedType about to lookup in deviceData...", detailedNotif,get_LOG_TRACE())
	if (objectId) {   
		traceEvent(settings.logFilter,"getObject>id=${objectId}, about to lookup $objectId...", detailedNotif,get_LOG_TRACE())
		try {
			foundObject = deviceData[objectId]
			if (foundObject) {
				if ((relatedType) && (relatedType != foundObject.type)) {
					traceEvent(settings.logFilter,"getObject>id=${objectId}, type (${foundObject?.type}) is not = relatedType=${relatedType}", detailedNotif, get_LOG_WARN())
				}
				objects << foundObject                
			}                
		} catch (Exception e) {
			traceEvent(settings.logFilter,"getObject>id=${objectId} with relatedType=${relatedType}, exception $e", detailedNotif, get_LOG_ERROR())
		            
		} 
	} else if (relatedType) {
    
		traceEvent(settings.logFilter,"getObject>About to loop to get all ${relatedType} objects in deviceData...", detailedNotif,get_LOG_TRACE())
...
//				traceEvent(settings.logFilter, "getObject>object map=${value.inspect()}\n", detailedNotif)              
				if (relatedType == value?.type) {
					traceEvent(settings.logFilter,"getObject>found ${value?.id},name=${value?.name} with relatedType=${relatedType}, objectType= ${value?.type}", detailedNotif,get_LOG_TRACE())
					objects << value                    
				}
			} else {
	...
	traceEvent(settings.logFilter,"getObject>end found ${objectId} with relatedType=${relatedType}, return= $objects", detailedNotif,get_LOG_TRACE())
	return objects
}
...
private void delete_obsolete_devices() {
    ...
    def structure_data =getStructures(false, settings.cache_timeout,'["buckets"]')
	def known_bucket_types= get_KNOWN_BUCKET_TYPES() 
	...
	if (old_sensorList && old_sensorList != new_camera_list) {
		...
		if (sensorsSet) {
			traceEvent(settings.logFilter, "MyNextManager>about to delete obsolete Sensors under ST: $sensorsSet", true, get_LOG_WARN(), true)
...
*/	
	...
	if (objectsToBeUpdated) child.updateChildData(objectsToBeUpdated)
...
private void delete_child_tstats(deleteTstats=[]) {
	...
	deleteTstats.each { 
		try {    
			...
			traceEvent(settings.logFilter,"deleteTstats>exception $e while deleting Nest thermostat ${it.deviceNetworkId},needs to be removed from all automation scenes & rooms", detailedNotif, get_LOG_ERROR())
		}   
	}
...
private void delete_child_sensors(deleteSensors=[]) {
	...
	deleteSensors.each { 
		try {    
			...
			traceEvent(settings.logFilter,"deleteTstats>exception $e while deleting Nest Sensor ${it.deviceNetworkId}, needs to be removed from all automation scenes & rooms", detailedNotif, get_LOG_ERROR())
		}   
	}
...
private void delete_child_protects(deleteProtects=[]) {
	deleteProtects.each { 
		try {    
			...
			traceEvent(settings.logFilter,"delete_child_protects>exception $e while deleting Nest Protect unit ${it.deviceNetworkId},needs to be removed from all automation scenes & rooms", detailedNotif, get_LOG_ERROR())
		}   
	}
}
...
private void delete_child_cameras(deleteCameras=[]) {
	deleteCameras.each { 
		try {   
			...
			traceEvent(settings.logFilter,"delete_child_cameras>exception $e while deleting Nest Camera ${it.deviceNetworkId},needs to be removed from all automation scenes & rooms", detailedNotif, get_LOG_ERROR())
		}   
	}
}
...
def askAlexaMQHandler(evt) {
	...
	switch (evt.value) {
		...
		traceEvent(settings.logFilter,"askAlexaMQHandler>new refresh value=$evt.jsonData?.queues", detailedNotif, get_LOG_TRACE())
		break
	}
}
...
def rescheduleIfNeeded(evt) {
	...
	if (lastPollTime != currentTime) {    
		...
		traceEvent(settings.logFilter,"rescheduleIfNeeded>last poll was  ${lastPollTimeInMinutes.toString()} minutes ago", detailedNotif, get_LOG_TRACE())
		takeAction()        
	}
	if (((atomicState?.poll["last"]?:0) + (delay * 60000) < currentTime)) {
		traceEvent(settings.logFilter,"rescheduleIfNeeded>scheduling takeAction in ${delay} minutes..", detailedNotif,get_LOG_TRACE())
		if (delay <5) {      
...
def takeAction() {
	...
	if (((atomicState?.poll["rescheduled"]?:0) + (delay * 60000)) < now()) {
		traceEvent(settings.logFilter,"takeAction>scheduling rescheduleIfNeeded() in ${delay} minutes..", detailedNotif, get_LOG_TRACE())
		unschedule()        
...
//	delete_obsolete_devices()
	traceEvent(settings.logFilter,"poll>about to call, type=[buckets]", detailedNotif,get_LOG_TRACE())
    def structures =getStructures(false, settings.cache_timeout,'["buckets"]')
//	structures=getStructures(false, settings.cache_timeout,'["where"]')
	def known_buckets= get_KNOWN_BUCKET_TYPES()   
	traceEvent(settings.logFilter,"poll>about to call, type=$known_buckets", detailedNotif,get_LOG_TRACE())
	structures=getStructures(false, settings.cache_timeout,"${known_buckets}")
	poll_protects()    
	poll_tstats()    
	poll_cameras()   
...
private void poll_protects() {
	...
	def devicesProtects = protectUnits.collect { dni ->
		def d = getChildDevice(dni)
		if (d) {       
			traceEvent(settings.logFilter,"poll_protects>Looping thru protects, found id $dni, about to poll", detailedNotif, get_LOG_TRACE())
			d.poll()
...
private void poll_sensors() {
	...
	def devicesSensors = sensors.collect { dni ->
		def d = getChildDevice(dni)
		if (d) {       
			traceEvent(settings.logFilter,"poll_sensors>Looping thru protects, found id $dni, about to poll", detailedNotif, get_LOG_TRACE())
			d.poll()
			exceptionCheck = d.currentVerboseTrace?.toString()
			if ((exceptionCheck) && (((exceptionCheck.contains("exception") || (exceptionCheck.contains("error"))) && 
				(!exceptionCheck.contains("TimeoutException")) && (!exceptionCheck.contains("No signature of method: physicalgraph.device.CommandService.executeAction")) &&
...
private void poll_tstats() {
	...
	def devicesTstats = thermostats.collect { dni ->
		def d = getChildDevice(dni)
		if (d) {       
			traceEvent(settings.logFilter,"poll_tstats>Looping thru thermostats, found id $dni, about to poll", detailedNotif, get_LOG_TRACE())
			d.poll()
			exceptionCheck = d.currentVerboseTrace?.toString()
			if ((exceptionCheck) && (((exceptionCheck.contains("exception") || (exceptionCheck.contains("error"))) && 
				(!exceptionCheck.contains("TimeoutException")) && (!exceptionCheck.contains("No signature of method: physicalgraph.device.CommandService.executeAction")) &&
...
//	send_msg=true    
	...
	traceEvent(settings.logFilter,"login>begin", detailedNotif, get_LOG_TRACE())
	...
	traceEvent(settings.logFilter,"login>google_issue_url=$issue_token_url", detailedNotif, get_LOG_TRACE())
	...
	try {
		httpGet(params) { resp ->
				atomicState?.lastHttpStatus=resp?.status
				traceEvent(settings.logFilter,"login>resp.status=${resp?.status}", detailedNotif, get_LOG_TRACE(),send_msg)
                
				if (resp?.data?.toString().contains("USER_LOGGED_OUT")) {
                
					traceEvent(settings.logFilter,"login>error=${resp?.data}, your home.nest.com session is no longer active, need to get new issue_token_url & cookie from Google account login, exiting...", detailedNotif, get_LOG_ERROR(),true)
					return false               
				}                
				if (resp?.status == NEST_SUCCESS) {
					traceEvent(settings.logFilter, "login>resp.data=${resp?.data}",detailedNotif,  get_LOG_TRACE(),send_msg)
					...
					traceEvent(settings.logFilter, "login>nest_access_token=${resp?.data}",detailedNotif,  get_LOG_TRACE(),send_msg)
				} else {
					traceEvent(settings.logFilter,"login>http status: ${resp?.status}",detailedNotif, get_LOG_TRACE(),send_msg)
				}    
			}                
	} catch (java.net.UnknownHostException e) {
		atomicState?.lastHttpStatus=e?.response?.status
	...
	traceEvent(settings.logFilter,"login>About to request jwt, httpPostParams=$httpPostParams", detailedNotif, get_LOG_TRACE(), send_msg)
	try {
		httpPost(httpPostParams) { resp ->
			atomicState?.lastHttpStatus=resp?.status
			msg="login>resp.data=${resp?.data}, status=${resp?.status}"
			traceEvent(settings.logFilter,msg, true,get_LOG_DEBUG(),send_msg)  
			if (resp?.status == NEST_SUCCESS) {        
				def jsonMap = resp?.data
				msg="login>google auth success, jsonMap=${jsonMap}"        
				traceEvent(settings.logFilter,msg, true,get_LOG_TRACE(),send_msg)  
...
private def get_KNOWN_BUCKET_TYPES() {

	return '["device","shared","topaz","kryptonite","quartz"]'
}

private def get_AppSettingsValue(variable) {
	return appSettings?."$variable".toString().trim()
}
